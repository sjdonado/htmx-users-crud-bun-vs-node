...
const UserListRow = (props: { user: User }) => {
  const fotmatDatetime = (date: string) => {
  ...
  return (
    <div id="users-list-row-{props.user.id}" class="border flex py-2">
      ...
      <span class="w-1/3 px-4 truncate" title="{formatDatetime props.user.createdAt}">
        {fotmatDatetime(props.user.createdAt)}
      </span>
